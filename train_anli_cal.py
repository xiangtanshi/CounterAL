...
class ANLI_AL:
    ...
    def AL_train(self):
        ...
        for r in range(0,self.qtotal): #if in ablation study for cal and badge, add -1 epoch for starting
            ...
            if r == 0:
                ...
                self.nli_train_start(trains, self.batchsize, l_indexs, un_indexs, cf_aug=self.cf, cf_loader=self.train_hyp_cf)
            else:
                self.nli_train(trains, self.batchsize, l_indexs, un_indexs, cf_aug=self.cf, cf_loader=self.train_hyp_cf)
    ...
    def finetune_nli(self):
        ...
        train_batch = self.trains.concate_batch(self.train_hyp_cf,self.batchsize,True,indexs)
        ...
        for i in range(self.epoch):
            ...
            for index in tqdm(range(len(train_batch))):
                ...
                if i>2:
                    ...
                    loss = loss_mask_rsc(self.model, embedding, label, self.device, 0.6)
                    loss.backward()
