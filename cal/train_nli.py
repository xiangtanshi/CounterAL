...
def get_args():
    ...
    parser.add_argument('--total',default=12,type=int,help='the total number of query rounds')
...
class NLI_AL:
    ...
    def AL_train(self):
        ...
        if self.cf and not self.cf_multi:
            augmented_labels = np.zeros(len(l_indexs))
            for i,lab in enumerate(self.cf_labels):
                augmented_labels[i] = lab
            np.savetxt('./record/{}/{}/{}label_cf.txt'.format(self.task,self.func,self.seed),augmented_labels)
