...
class dataset:
    ...
    def get_item_3(self, index, cf_label=None):
    ...
    def get_item_4(self, index, cf_label):
        ...
        if label_cf1 == cf_label:
            ...
            sentence_cf = [(self.data['sentence1'][2*index+1],self.data['sentence2'][2*index+1])]
    ...
    def concate_batch(self, Datas, batchsize=1, shuffle=False, indexs=None, cf_labels=None):
        ...
        batch = []
        if not indexs:
            raise ValueError('indexs is null, but should be given.')
        else:
            size = len(indexs)
            if shuffle:
