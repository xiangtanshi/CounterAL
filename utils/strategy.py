...
def LeastConfidenceSampling(tokenizer, model, l_indexs, un_indexs, loader, size, device):
    ...
    with torch.no_grad():
        ...
        for index in un_indexs:
            ...
            probs_log = probs.log()
            entropy = float(-torch.sum(probs * probs_log))
            max_confidence_list.append((index,entropy))
...
def CALSampling(tokenizer, model, l_indexs, un_indexs, loader, size, device):
    ...
    un_embeddings = np.zeros((len(un_indexs),768))
    l_embeddings = np.zeros((len(l_indexs),768))
    un_logits = torch.zeros(len(un_indexs),num_class)
    l_logits = torch.zeros(len(l_indexs),num_class)
    with torch.no_grad():
        for i,index in enumerate(l_indexs):
            _,sentence = loader.GetItem(index)
            sentence = sentence[0:1]
            ...
            input_ids = encoding["input_ids"].to(device)
            attention_mask = encoding['attention_mask'].to(device)
            embedding = model.roberta(input_ids,attention_mask).last_hidden_state[0:1,0:1,:]
            logit = F.softmax(model.classifier(embedding),dim=1)
            embedding = embedding.cpu()
            logit = logit.cpu()
            l_embeddings[i] = embedding[0,0]
            l_logits[i] = logit[0]
    ...
    un_logits = un_logits.log()
    kl_score = np.zeros(len(un_indexs))
    for i in range(len(un_indexs)):
        kl_score[i] = sum([criterion(un_logits[i],l_logits[j]) for j in un_knn[i]])
