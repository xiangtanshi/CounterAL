...
def get_args():
    ...
    parser.add_argument('--func',type=str,default='badge_aug',help='the query strategy that we take')
...
class ANLI_AL:
    ...
    def CounterALSampling(self, un_indexs, loader):
            ...
            # diversified
            candidates = [i[0] for i in variance[0:400]]
        ...
        while(end == 0):
            ...
            for index in range(len(L_batch)):
                ...
                if l_acc < 1:
                    ...
                    loss = loss_mask(self.model, embedding, label, self.device, threshold=0.03)
                    loss.backward()
    ...
    def AL_train(self):
        ...
        for r in range(-1,self.qtotal): #if in ablation study for cal and badge, add -1 epoch for starting
            ...
            if r < 0:
                ...
                # q_idx = CALSampling(self.tokenizer, self.model, l_indexs, candidates, trains, self.qsize, self.device, self.train_hyp_cf)
            ...
            # update
            if r == -1:
                self.nli_train_start(self.train_ori, 4, l_indexs, un_indexs, cf_aug=False, cf_loader=None) # for cal and badge
                # self.nli_train_start(trains, self.batchsize, l_indexs, un_indexs, cf_aug=self.cf, cf_loader=self.train_hyp_f)
            else:
                self.nli_train(trains, self.batchsize, l_indexs, un_indexs, cf_aug=self.cf, cf_loader=self.train_hyp_f)
    ...
    def finetune_nli(self):
        ...
        train_batch = self.trains.concate_batch(self.train_hyp_f,self.batchsize,True,indexs)
